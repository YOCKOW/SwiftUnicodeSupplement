...
module DerivedBinaryProperties
  ...
  module_function def write(remote_io, file)
    ...
    raise "No data about DerivedBinaryProperties" if table.count < 1
    ...
    arranged_table.each{|prop, integers|
      ...
      file.puts("internal let _binProp_#{prop} = _UnicodePredicate(#{array_identifier},alreadySorted:true)")
    }
  end
end
